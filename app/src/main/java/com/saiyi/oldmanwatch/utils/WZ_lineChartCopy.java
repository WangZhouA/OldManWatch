...
public class WZ_lineChartCopy {
    ...
    public  void initChartView(Context context,LineChart mChart, final List<String>  valuesNew) {
...
//        mChart.setDragDecelerationFrictionCoef(0.9f);
        mChart.setBackgroundColor(context.getResources().getColor(R.color.lineChart_bg));

        mChart.setViewPortOffsets(100, 50, 100, 100);
...
//
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            ...
            public String getFormattedValue(float value, AxisBase axis) {
                int index = (int) value;
                if (index < 0 || index >= valuesNew.size()) {
                    return "";
                } else {
                    return valuesNew.get(index);
                }
            }
        });
...
//        mChart.setVisibleXRange(0, 5);
    ...
    public   void initDataDan(ArrayList<Entry> values1 ,  LineChart mChart , int color_wz ) {
...
//        float ratio = (float) values1.size()/(float)6;
