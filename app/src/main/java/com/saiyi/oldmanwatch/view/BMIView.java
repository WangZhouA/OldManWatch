...
public class BMIView extends View {
    ...
    private int minVal, minNormal, centerNormal, maxNormal, maxVal;
    private float currVal;
    private String currStr="";
    private int leftColor, midColor, centerColor, rightColor, normalTextColor;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        smallCircleRadius = (int) array.getDimension(R.styleable.BMIView_circleRadius, 15);
        circleStokeWidth = (int) array.getDimension(R.styleable.BMIView_circleStokeWidth, 40);
        ...
        leftColor = array.getColor(R.styleable.BMIView_leftColor, Color.parseColor("#895CF0"));
        midColor = array.getColor(R.styleable.BMIView_midColor, Color.parseColor("#2ADF76"));
        centerColor = array.getColor(R.styleable.BMIView_rightColor, Color.parseColor("#FFAA00"));
        rightColor = array.getColor(R.styleable.BMIView_rightColor, Color.parseColor("#FF5E3C"));

        normalTextColor = array.getColor(R.styleable.BMIView_normalTextColor, Color.GRAY);
        lineWidth = (int) array.getDimension(R.styleable.BMIView_lineWidth, 20);
    ...
    public void setCenterNormal(int centerNormal) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int offsetX = 0, offsetY = height / 4, window = width / 4;
        ...
        if (currVal <= minNormal) {
            // 在左边部分
            int x = (int) ((currVal / (double) minNormal) * width / 4);
            Log.e("左x==", x + "");
            ...
            // 当前值在右边部分
            int x = width / 4 * 3 + (int) (((currVal - maxNormal) / (double) (maxVal - maxNormal)) * width / 3);
            int y = offsetY + height / 3 - lineWidth / 2;
            ...
            // 当前值在中间部分
            int x = width / 3 + (int) ((currVal - minNormal) / (double) (maxNormal - minNormal) * (width / 2.5)) - smallCircleRadius;
            int y = offsetY + height / 3 - lineWidth / 2;
            linePaint.setColor(Color.WHITE);
            linePaint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(x, y, smallCircleRadius, linePaint);
        ...
        canvas.drawText(String.valueOf(minNormal), width / 4 - textWidth / 2, offsetY + height / 3 + lineWidth * 4 + normalTextSize / 2, textPaint);

        textWidth = textPaint.measureText(String.valueOf(maxNormal));
        canvas.drawText(String.valueOf(maxNormal), width / 4 * 3 - textWidth / 2, offsetY + height / 3 + lineWidth * 4 + normalTextSize / 2, textPaint);
    }
}
